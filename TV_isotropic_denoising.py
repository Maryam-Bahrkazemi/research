...
def itv_norm(u):
    u = np.sum(np.sqrt(np.square(gradient_x(u)) + np.square(gradient_y(u))))
    return u
############################
...
l = 2*mu
alf = 0.1
#######################Split Bregman algorithm for image denoising using isotropic total variation
u = f
bx = np.ones_like(u_s, dtype=float)
...
for k in range(n_iter):
    #the u subproblem, using gradient descent algorithm
    ...
    #the d subproblem applying shrink function
    dx = shrink(grad_ux + bx, l)
    dy = shrink(grad_uy + by, l)

    #updating the bregamn parameters
    bx += (grad_ux - dx)
    by += (grad_uy - dy)

    #the amount of total variation and data fidelity in each iterations
    tv.append(itv_norm(u))
    df.append(l2_square(u))

print('tv_original_image:', itv_norm(u_s))
print('tv_noisy_image:', itv_norm(f))
print('tv_denoised_image:', itv_norm(u))
